...
class MessageMapper(mapper):
    ...
    def find_by_key(self, key):
        ...
        cursor = self._connection.cursor()
        command = f'SELECT message_id, sender_id, recipient_id, timestamp, content FROM Message WHERE message_id={key}'
        cursor.execute(command)
        tuples = cursor.fetchall()

        if tuples is not None and len(tuples) > 0 and tuples[0] is not None:
            (message_id, sender_id, recipient_id, timestamp, content) = tuples[0]
            message = Message()
            message.set_id(message_id)
            message.set_sender(sender_id)
            message.set_recipient(recipient_id)
    ...
    def insert(self, message):
        ...
        command = 'INSERT INTO Message (message_id, sender_id, recipient_id, timestamp, content) VALUES (%s, %s, %s, ' \
                  '%s, %s)'
        data = (message.get_id(),
                message.get_sender(),
